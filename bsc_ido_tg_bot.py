...
def _decode_create_ido_input_with_abi(input_data: str) -> Tuple[List[str], List[int]]:
    """
    用 ABI decoder 尝试解析常见 createIDO 参数布局。
    兼容：
      1) (address[] _addresses, uint256[] _startAndEndTimestamps, uint8 _maxPoolId)
      2) (address[] _addresses, uint256[] _startAndEndTimestamps)
    """
    if not input_data or input_data == "0x" or len(input_data) < 10:
        return [], []
    try:
        payload = bytes.fromhex(input_data[10:])  # 去掉 0x + 4byte selector
    except Exception:
        return [], []
    ...
    for abi_types in candidate_types:
        try:
            decoded = w3.codec.decode(abi_types, payload)
        except Exception:
            continue
        if len(decoded) < 2:
            continue
        raw_addrs = decoded[0] or []
        raw_ts = decoded[1] or []
        addrs: List[str] = []
        for a in raw_addrs:
            try:
                addrs.append(checksum_address(str(a)))
            except Exception:
                addrs.append(str(a))
        ts = [int(x) for x in raw_ts if 1483228800 <= int(x) <= 2208988800]
        return addrs, ts
    return [], []
...
def extract_tx_extra_info(tx_hash: str) -> Tuple[Optional[str], Optional[str], Optional[int], Optional[int]]:
    ...
    # 优先使用 ABI decoder，失败再走手工解码
    decoded_addrs, decoded_ts = _decode_create_ido_input_with_abi(input_data)
    if not decoded_addrs and not decoded_ts:
